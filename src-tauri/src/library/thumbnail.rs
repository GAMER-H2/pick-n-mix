...
/// JPEG quality for generated thumbnails. Well above the encoder's default of
/// 75: these are covers, viewed at the size they were made for on displays
/// dense enough to show the artefacts, and the extra bytes cost a few hundred
/// KB of cache rather than any decode time.
const QUALITY: u8 = 92;

/// Bumped whenever generation changes in a way that makes already-cached
/// files worse than what would be produced now — the filter, the quality, the
/// format. Cached thumbnails live under this version, and older versions are
/// deleted at startup, so a change actually reaches covers already seen
/// instead of only ones scanned afterwards.
const CACHE_VERSION: u32 = 3;

fn thumbs_dir(artwork_dir: &Path) -> PathBuf {
    artwork_dir.join("thumbs").join(format!("v{CACHE_VERSION}"))
}

/// Drop thumbnails generated by an older version of the code above.
fn prune_old_versions(artwork_dir: &Path) {
    let current = thumbs_dir(artwork_dir);
    let Ok(entries) = std::fs::read_dir(artwork_dir.join("thumbs")) else {
        return;
    };
    for entry in entries.flatten() {
        ...
        // Pre-versioning caches wrote their files directly into `thumbs/`.
        if path.is_dir() {
...
fn thumb_path(artwork_dir: &Path, id: &str, width: u32) -> PathBuf {
    thumbs_dir(artwork_dir).join(format!("{id}-{width}.jpg"))
}
...
fn marker_path(artwork_dir: &Path, id: &str, width: u32) -> PathBuf {
    thumbs_dir(artwork_dir).join(format!("{id}-{width}.orig"))
}
...
fn generate(artwork_dir: &Path, id: &str, original: &Path, width: u32) {
    let thumbs_dir = thumbs_dir(artwork_dir);
    ...
    // An animated cover has no still equivalent: every frame after the first
    // would be thrown away, so a "thumbnail" of it is the wrong picture held
    // still. The original is the right answer at every size.
    ...
    // Lanczos3 rather than a cheaper bilinear filter: this runs once per
    // cover per size, off any request, and bilinear downscaling by the large
    // factors involved here (a 3000px cover to 256px) is what makes a
    // thumbnail look soft next to the original.
    ...
    if write_jpeg(&tmp, &resized.to_rgb8()).is_err() {
...
/// Whether this file is an animated image, from its header alone — cheap
/// enough to run per job, and it trusts the bytes rather than the extension
/// the cover happened to be stored under.
fn is_animated(path: &Path) -> bool {
    ...
    match reader.format() {
        // Telling a one-frame GIF from an animation means decoding the whole
        // thing; a still GIF is rare, and all it loses is its thumbnail.
        Some(image::ImageFormat::Gif) => true,
        Some(image::ImageFormat::WebP) => webp_is_animated(path),
        _ => false,
    }
}

/// A WebP declares animation in its extended-format header: `VP8X` as the
/// first chunk, with bit 1 of the following flags byte set. All fixed offsets,
/// so this reads a 21-byte prefix and nothing else.
...
/// Encodes at `QUALITY`, which `save_with_format` gives no way to choose.
fn write_jpeg(path: &Path, image: &image::RgbImage) -> Result<(), image::ImageError> {
    let file = std::fs::File::create(path)?;
    let mut writer = std::io::BufWriter::new(file);
    image::codecs::jpeg::JpegEncoder::new_with_quality(&mut writer, QUALITY).encode_image(image)
}
...
impl Thumbnailer {
    pub fn new(artwork_dir: PathBuf) -> Self {
        prune_old_versions(&artwork_dir);
...
mod tests {
    ...
    /// A quality change is only worth making if it reaches covers already
    /// cached, which means the old files have to go.
    #[test]
    fn starting_up_drops_thumbnails_from_an_older_cache_version() {
        let dir = tempfile_dir();
        let artwork_dir = dir.join("artwork");
        let stale_dir = artwork_dir.join("thumbs").join("v1");
        std::fs::create_dir_all(&stale_dir).unwrap();
        std::fs::write(stale_dir.join("abc-256.jpg"), b"old").unwrap();
        // The pre-versioning layout, which wrote straight into `thumbs/`.
        std::fs::write(artwork_dir.join("thumbs").join("abc-256.jpg"), b"older").unwrap();

        // A current-version file has to survive the same sweep.
        std::fs::create_dir_all(thumbs_dir(&artwork_dir)).unwrap();
        let keep = thumb_path(&artwork_dir, "abc", 256);
        std::fs::write(&keep, b"current").unwrap();

        let _thumbnailer = Thumbnailer::new(artwork_dir.clone());

        assert!(!stale_dir.exists());
        assert!(!artwork_dir.join("thumbs").join("abc-256.jpg").exists());
        assert!(keep.exists());

        std::fs::remove_dir_all(&dir).ok();
    }

    /// The animation is the whole point of an animated cover, so it is served
    /// whole at every size rather than frozen into a still.
    #[test]
    fn never_thumbnails_an_animated_cover() {
        let dir = tempfile_dir();
        let original = dir.join("cover.gif");
        // Two frames, comfortably over the bucket it is asked for.
        let mut bytes = Vec::new();
        {
            ...
            for shade in [40u8, 200u8] {
                let frame = image::RgbaImage::from_pixel(200, 200, image::Rgba([shade, 0, 0, 255]));
                encoder.encode_frame(image::Frame::new(frame)).unwrap();
            }
        }
        std::fs::write(&original, &bytes).unwrap();

        assert!(is_animated(&original));
        generate(&dir, "gif", &original, 64);
        assert!(matches!(lookup(&dir, "gif", 64), Some(Art::Original)));

        std::fs::remove_dir_all(&dir).ok();
    }

    /// A still image of any format still gets the thumbnail it always did.
    #[test]
    fn a_still_cover_is_not_mistaken_for_an_animated_one() {
        let dir = tempfile_dir();
        let original = dir.join("cover.png");
        image::RgbImage::from_pixel(400, 400, image::Rgb([5, 6, 7]))
            .save(&original)
            .unwrap();

        assert!(!is_animated(&original));
        generate(&dir, "still", &original, 64);
        assert!(matches!(lookup(&dir, "still", 64), Some(Art::Thumb(_))));
